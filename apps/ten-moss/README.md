...
The integration is powered by the [`ten-moss`](../../packages/ten-moss) package (`MossSessionManager`) and lives entirely in the `main_python` control extension.
...
2. **Drop the app into a TEN checkout.** Copy `tenapp/` to `ten-framework/ai_agents/agents/examples/voice-assistant-with-moss/tenapp/`, alongside the sibling `voice-assistant` example whose Taskfile, playground, and server harness you reuse. `main_python` depends on [`ten-moss`](https://pypi.org/project/ten-moss/) (listed in `main_python/requirements.txt`), so `task install` pulls it from PyPI automatically.

3. **Run with TEN's tooling** from that example directory (`task install && task run`, per the TEN docs), providing the same env vars as step 1. Open the TEN playground at http://localhost:3000 and ask something covered by `data/knowledge.jsonl`, for example *"how long do refunds take?"*, to hear grounded answers.

## Compare answers with and without Moss

No voice stack needed. `compare.py` answers the same questions with the same LLM twice, once without Moss and once with the exact grounding the agent injects, so you can see the improvement without standing up the full pipeline. It needs only your Moss and OpenAI keys (no Agora, STT, or TTS):
...
| --- | --- | --- |
| How long do refunds take? | "5-10 business days" (wrong) | "3-5 business days once approved" (correct) |
| Can I cancel my order? | "within a specific timeframe... check our policy" (vague) | "within 1 hour of placement" (correct) |
| Which payment methods? | misses American Express | "Visa, Mastercard, Amex, PayPal, Apple Pay" (correct) |
| Do you offer price matching? | "provide competitor details" (vague) | "authorized retailers within 14 days" (correct) |
| How fast is express shipping? | "1-3 business days" (wrong) | "1-2 business days" (correct) |

Without grounding, the model confidently invents plausible but wrong specifics; with Moss, it answers from your knowledge base. This is the exact delta the live voice agent applies per turn. Flip `enable_moss` in `property.json` to A/B the same thing in the playground.
...
TEN's shipped memory/RAG backends (memU, OceanBase PowerRAG, EverMemOS) are all remote services, so every turn pays a network round trip. To compare TEN's default (memU) against Moss with real logged latency from both agents, follow **[`BENCHMARK.md`](BENCHMARK.md)**: it adds the same one-line latency log to TEN's shipped `voice-assistant-with-memU` example, then you run both agents and read `[retrieval-latency]` from each (memU: hundreds of ms; Moss: single-digit ms).

### Simulate a remote store (no memU key needed)
