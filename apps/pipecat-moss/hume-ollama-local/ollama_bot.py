...
"""Pipecat Voice AI Bot with Ollama (local LLM) + Moss Semantic Retrieval + TADA MLX TTS.
...
- Uses Hume AI TADA (MLX) for fully local text-to-speech on Apple Silicon
...
- TADA MLX (local TTS, no API key needed)
...
from tada_tts import TadaMLXTTSService
...
print("Starting Ollama + TADA MLX Voice Bot...")
logger.debug("All components loaded successfully!")

async def run_bot(transport: BaseTransport, runner_args: RunnerArguments):
    """Run the voice bot pipeline with Ollama LLM and TADA MLX TTS."""

    # Initialize stt, tts, llm services
    logger.debug("Starting Ollama + TADA MLX voice bot")
    ...
    # TADA MLX — fully local TTS on Apple Silicon
    reference_path = os.path.join(os.path.dirname(__file__), "reference_voice.wav")
    tts = TadaMLXTTSService(
        model_id=os.getenv("TADA_MODEL_ID", "HumeAI/mlx-tada-1b"),
        reference_audio_path=reference_path,
        reference_text=os.getenv("TADA_REFERENCE_TEXT"),
        quantize=int(os.getenv("TADA_QUANTIZE", "4")),
        flow_matching_steps=int(os.getenv("TADA_FLOW_STEPS", "6")),
    )
    # Eagerly load model + pre-warm Metal kernels before any client connects
    tts.load()
...
async def bot(runner_args: RunnerArguments):
    """Main bot entry point for the Ollama + TADA MLX voice bot."""
    ...
    if missing_vars:
        ...
        logger.error("TADA MLX and Ollama run locally — no API keys needed")
        return
