    ...
    async def on_user_turn_completed(
        ...
        try:
            ...
            results = await self.moss.query(
                INDEX_NAME, user_query, QueryOptions(top_k=5, alpha=0.8)
            )
            ...
            if results.docs:
                ...
                logger.info(
                    f"Injected context: {context_str[:100]}..."
                )  # Log first 100 chars
            else:
...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        logger.warning(
            "Moss queries will fail until the index is created. Run upload.py first."
        )
