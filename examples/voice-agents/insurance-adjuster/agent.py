...
class AdjusterAgent(Agent):
    ...
    async def on_user_turn_completed(self, turn_ctx: ChatContext, new_message: ChatMessage) -> None:
        ...
        if data.policy_session:
            tasks.append(data.policy_session.query(query, QueryOptions(top_k=3, alpha=0.75)))
        if data.claim_session and data.claim_session.doc_count > 0:
            tasks.append(data.claim_session.query(query, QueryOptions(top_k=3, alpha=0.7)))
        ...
        findings_result = results[2] if len(results) > 2 and not isinstance(results[2], Exception) else None
        ...
        if policy_result and policy_result.docs:
            ...
            turn_ctx.add_message(role="system", content=(
                f"Policy context for {data.policy}:\n---\n{lines}\n---"
            ))
        ...
        if findings_result and findings_result.docs:
            logger.info("findings session: %d docs in %dms",
                        len(findings_result.docs), findings_result.time_taken_ms)
            lines = "\n".join(f"- {d.text}" for d in findings_result.docs)
            turn_ctx.add_message(role="system", content=(
                f"Findings logged so far this inspection:\n---\n{lines}\n---"
            ))
    ...
    async def log_finding(
        ...
        if data.claim_session:
            await data.claim_session.add_docs([DocumentInfo(
                id=f"finding-{idx}",
        ...
        logger.info("finding %d: %s, $%.0f, %s", idx, description[:60], estimated_value, status)
    ...
    async def submit_report(self, _context: RunContext) -> str:
        ...
        if data.claim_session:
            try:
                ...
                logger.info("claim session pushed: %s (job %s)", session_ref, result.job_id)
            except Exception as e:
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    if policy:
        ...
        if session:
            ...
            logger.warning("policy %s not in pre-warmed set, agent will ask verbally", clean)
