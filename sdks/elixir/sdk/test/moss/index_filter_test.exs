defmodule Moss.IndexFilterTest do
  @moduledoc """
  Filter parity tests for Moss.Session.query/3.
  Counterpart to test_index_filter.py in the Python SDK.

  Uses model_id: "custom" with pre-computed embeddings — no ONNX runtime needed.
  Requires cloud credentials for session construction.
  """
  ...
  defp build_session(name, doc_emb_pairs) do
    {:ok, pid} =
  ...
  defp query_ids(pid, filter) do
    {:ok, result} = Moss.Session.query(pid, "test", embedding: query_emb(), top_k: 10, alpha: 0.8, filter: filter)
  ...
  describe "filter conditions" do
    setup do
      pid = build_session("idx-filter-cond-#{System.unique_integer([:positive])}", filter_docs())
      %{pid: pid}
    end
  ...
  describe "composite filters" do
    setup do
      pid = build_session("idx-filter-comp-#{System.unique_integer([:positive])}", filter_docs())
      %{pid: pid}
    end
  ...
  describe "edge cases" do
    setup do
      pid = build_session("idx-filter-edge-#{System.unique_integer([:positive])}", filter_docs())
      %{pid: pid}
    end
    ...
    test "nil filter returns all docs", %{pid: pid} do
      {:ok, result} = Moss.Session.query(pid, "test", embedding: query_emb(), top_k: 10, alpha: 0.8)
  ...
  describe "$near geo filter" do
    setup do
      pid = build_session("idx-geo-#{System.unique_integer([:positive])}", geo_docs())
      %{pid: pid}
    end

    test "10km radius matches ts and sol", %{pid: pid} do
      filt = %{"field" => "location", "condition" => %{"$near" => "40.7580,-73.9855,10000"}}
      {:ok, result} = Moss.Session.query(pid, "test", embedding: query_emb(), top_k: 10, alpha: 0.8, filter: filt)
    ...
    test "5km radius excludes sol (~8.7km)", %{pid: pid} do
  ...
  describe "hybrid search + filter" do
    setup do
      pid = build_session("idx-hybrid-#{System.unique_integer([:positive])}", hybrid_docs())
      %{pid: pid}
    end
