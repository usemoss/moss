...
def list_profiles() -> list[str]:
    ...
    if not isinstance(profiles, dict):
        return []
    return sorted(name for name in profiles if isinstance(name, str) and name)
...
def delete_profile(profile: str) -> tuple[bool, str | None]:
    ...
    active_profile = normalized.get("active_profile")
    remaining_profiles = sorted(name for name in profiles if isinstance(name, str) and name)
    if (
