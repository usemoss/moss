---
search: false
---

[**@moss-dev/moss-web v1.0.0-beta.7**](README.md)

***

# @moss-dev/moss-web v1.0.0-beta.7

## Classes

- [MossClient](classes/MossClient.md)

## Interfaces

- [ModelRef](interfaces/ModelRef.md)
- [IndexInfo](interfaces/IndexInfo.md)
- [DocumentInfo](interfaces/DocumentInfo.md)
- [QueryResultDocumentInfo](interfaces/QueryResultDocumentInfo.md)
- [SearchResult](interfaces/SearchResult.md)
- [QueryOptions](interfaces/QueryOptions.md)
- [GetDocumentsOptions](interfaces/GetDocumentsOptions.md)
- [JobProgress](interfaces/JobProgress.md)
- [CreateIndexOptions](interfaces/CreateIndexOptions.md)
- [MutationOptions](interfaces/MutationOptions.md)
- [MutationResult](interfaces/MutationResult.md)
- [JobStatusResponse](interfaces/JobStatusResponse.md)
- [LoadIndexOptions](interfaces/LoadIndexOptions.md)

## Type Aliases

- [ISODate](type-aliases/ISODate.md)
- [MossModel](type-aliases/MossModel.md)
- [FilterCondition](type-aliases/FilterCondition.md)
- [MetadataFilter](type-aliases/MetadataFilter.md)
- [JobStatus](type-aliases/JobStatus.md)
- [JobPhase](type-aliases/JobPhase.md)
